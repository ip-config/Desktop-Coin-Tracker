...
(function($) {
	...
	FlipClock.List = FlipClock.Base.extend({
		...
		*/
		...
		select: function(value) {
			...
			if (this.value != this.getOption("lastValue")) {
				...
				this.$el
					.find("." + this.getOption("classes").active)
					.removeClass(this.getOption("classes").active)
					.addClass(this.getOption("classes").before);

				this.items.splice(0, 1);

				this._afterListItem = this.createListItem(
					this.value,
					this.getOption("classes").active
				);
		...
		*/
		...
		createList: function() {
			var $el = (this.$el = $(
				'<ul class="' + this.getOption("classes").flip + '"></ul>'
			));

			this._beforeListItem = this.createListItem(
				this.getPrevValue(),
				this.getOption("classes").before
			);
			this._afterListItem = this.createListItem(
				this.value,
				this.getOption("classes").active
	...
	FlipClock.EnglishAlphaList = FlipClock.List.extend({
		...
		*/
		...
		getPrevValue: function() {
			...
			var minCode = this.getMinCharCode(),
				maxCode = this.getMaxCharCode();
		...
		*/
		...
		constructor: function(options) {
			...
			this.$el = $(
				[
					'<span class="' +
						this.getOption("classes").divider +
						" " +
						(this.getOption("css") ? this.getOption("css") : "").toLowerCase() +
						'">',
					'<span class="' +
						this.getOption("classes").label +
						'">' +
...
(function($) {
	...
	FlipClock.Factory = FlipClock.Base.extend({
		...
		constructor: function($el, value, options) {
			...
			this.loadClockFace(
				this.getOption("clockFace"),
				value,
				this.getOption("clockFaceOptions")
			);
		...
		*/
		...
		loadClockFace: function(name, value, options) {
			...
			if (FlipClock[name]) {
				...
				this.face = new FlipClock[this.getOption("defaultClockFace") + suffix](
					value,
					options
				);
			}
	...
	FlipClock.Time = FlipClock.Base.extend({
		...
		*/
		...
		getHourCounter: function(includeSeconds) {
			var data = [this.getHours(), this.getMinutes(true)];
		...
		getTimeSeconds: function(countdown, date) {
			...
			if (this.time instanceof Date) {
				if (countdown) {
					return Math.round(
						Math.max(this.time.getTime() / 1000 - date.getTime() / 1000, 0)
					);
				} else {
	...
	FlipClock.Uuid = FlipClock.Base.extend({
		...
		*/
		...
		isUuid: function(value) {
			...
			return (
				value && (value instanceof Uuid || validator.test(value.toString()))
			);
		},
...
(function($) {
	...
	FlipClock.TwentyFourHourClockFace = FlipClock.Face.extend({
		...
		build: function(time) {
			var time = time
				? time
				: this.time.getMilitaryTime(false, this.getOption("showSeconds"));

			for (var i in time) {
				this.createList(time[i]);
			}

			this.createDivider().$el.insertBefore(
	...
	FlipClock.DailyCounterFace = FlipClock.Face.extend({
		...
		build: function() {
			...
			if (this.getOption("showSeconds")) {
				this.createDivider("Seconds").$el.insertBefore(
					this.lists[this.lists.length - 2].$el
				);
			} else {
				offset = 2;
			...
			this.createDivider("Minutes").$el.insertBefore(
				this.lists[this.lists.length - 4 + offset].$el
			);
			this.createDivider("Hours").$el.insertBefore(
		...
		increment: function() {
			...
			while (flip) {
				...
				if (list) {
					...
					values.unshift(
						String.fromCharCode(this.getListObject(false).getMinCharCode())
					);
				}
			}
	...
	FlipClock.HourlyCounterFace = FlipClock.Face.extend({
		...
		build: function(time) {
			var offset = 0,
				time = time
					? time
					: this.time.getHourCounter(this.getOption("showSeconds"));
			...
			if (this.getOption("showSeconds") === true) {
				offset = 2;
				this.createDivider("Seconds").$el.insertBefore(
					this.lists[this.lists.length - offset].$el
				);
			}

			this.createDivider("Minutes").$el.insertBefore(
				this.lists[this.lists.length - 2 - offset].$el
			);
...
(function($) {
	...
	FlipClock.TwelveHourClockFace = FlipClock.TwentyFourHourClockFace.extend({
		...
		build: function() {
			var t = this,
				time = this.time.getTime(false, this.getOption("showSeconds"));
